const listenGuessRecentScore = localStorage.getItem('listenGuessRecentScore');
...
finalScore.innerText = `${listenGuessRecentScore}`;
if (listenGuessRecentScore <= SCORE_FIFTY) {
  message.innerText = "You've failed! Try harder";
  finalScore.style.backgroundImage = 'linear-gradient(#912e27d3,#db5230f1)';
} else if (listenGuessRecentScore > SCORE_FIFTY && listenGuessRecentScore <= SCORE_SEVENTY_FIVE) {
